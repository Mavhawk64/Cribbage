...
public class Game {
    ...
    private void startGame() {
        ...
        seconds %= 60;
        System.out
                .println("The game lasted " + hours + " hours, " + minutes + " minutes, and " + seconds + " seconds.");
    }

    /**
     * The owner of the crib is the dealer (starting with player 1, then 2 and so
     * on)
     ...
     * 4. The player to the left of the dealer goes first and announces the value of
     * their card
     ...
     * * 5c. If the player can't play a card to reach a sum at or under 31, they say
     * "Go", the previous player gets 1 point,
     * they can continue the sum to gain any extra points, and upon reaching or
     * breaching 31, they gain their points, and the next player plays at 0.
     ...
     * * 6c. Three-of-a-Kind: For adding a card that makes a three-of-a-kind, peg 6
     * points.
     * * 6d. Four-of-a-Kind: For adding a card that makes a four-of-a-kind, peg 12
     * points.
     * * 6e. Runs: For adding a card that makes a run of 3 or more cards, peg 1
     * point for each card in the run.
     * * * 6ei. If a run includes a pair, it counts as a double run, plus a pair.
     * * * 6eii. If a run includes a three-of-a-kind, it counts as a triple run,
     * plus a three-of-a-kind.
     * * * 6eiii. If a run includes a four-of-a-kind, it counts as a quadruple run,
     * plus a four-of-a-kind.
     * * * 6eiv. NOTE: A run made during the pegging phase doesn't have to be in
     * order. For example, if there is a 3, 4, and 5, the 5 can be played first,
     * then the 3, then the 4.
     * * 6f. Flush: Four or more cards of the same suit in your hand.
     * * * 6fi. For a flush of 4 cards, peg 4 points. If the start card is also the
     * same suit, peg 5 points.
     * * * 6fii. However, in the crib, only a flush of 5 cards counts. If the start
     * card is also the same suit, peg 5 points.
     * * * 6fiii. NOTE: A flush doesn't happen during the play of cards; it occurs
     * only when the hands and crib are counted.
     * * 6g. His Nobs: If the start card is a Jack, and it matches the suit of a
     * card in your hand or the crib, peg 1 point.
     * * 6h. Last Card: If you play the last card in your hand, peg 1 point.
     * 7. Counting the Hands
     * * 7a. When play ends, the hands of cards are counted in order: non-dealer's
     * hand, dealer's hand, crib.
    ...
    public void play() {
        ...
        while (!this.board.isGameOver()) {
            this.round++;
            // Decide the dealer (crib owner), deal cards to each player, and determine the
            // start card
            ...
            // The player to the left of the dealer goes first and announces the value of
            // their card,
            ...
            while (this.deck.playersStillHaveCards() && !this.board.isGameOver()) {
                ...
                for (int x = 0; x < this.players; x++) {
                    ...
                    // If the player can't play a card, they say "Go", the previous player gets 1
                    // point,
                    // they can continue the sum to gain any extra points, and upon reaching or
                    // breaching 31,
                ...
                // If no one can play a card, but there still are cards, restart the loop with a
                // clear pile
                if (goEnd) {
                    this.deck.wipePile();
                }
